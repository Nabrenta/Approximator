...
namespace Solver
{
    ...
    /// </summary>
    public enum Lines { Lagrang, MinSqear3Polinom }

    /// <summary>
    /// Static class contains mathematical alhorythms
    /// </summary>
    public static class Solver
    {
        /// <summary>
        /// Power of approximation polinom (3 according to the reqirement)
        /// </summary>
        ...
        /// </summary>
        /// <param name="soursData">Dots for approximation
        /// / Key = x
        /// / Value = y
        /// </param>
        ...
        {
            /*
             We need to approximate our dots to function f = c1 * x^n + c2 * x^(n-1) + ... + cn. We finde our c from  equations system:
               c1              c2        ...       cn
             [ sum(x^(2n - 2)) sum(x^(2n - 3)) ... sum(x^(n - 1)) | sum(y*x^(n - 1)) ]
             [ sum(x^(2n - 3)) sum(x^(2n - 4)) ... sum(x^(n - 2)) | sum(y*x^(n - 2)) ]
             [                         ...                        |        ...       ]
             [ sum(x^(n-1))    sum(x^(n-2))    ... n              | sum(y)           ]
            ...
            */
            ...
            return new KeyValuePair<Lines, SortedDictionary<double, double>>(Lines.MinSqear3Polinom, PolinomCreator(coeff, start, step, end));
        }
        ...
        private static SortedDictionary<double, double> PolinomCreator(double[] coeffitient, double start, double step, double end)
        {
        ...
        /// <summary>
        /// Create range of dots found using Lagrange polinom
        /// </summary>
        /// <param name="input">
        /// Sours data
        /// / Key = x
        ...
        /// <param name="end">End of range</param>
        /// <returns>Range</returns>
        public static KeyValuePair<Lines, SortedDictionary<double, double>> Lagrange(SortedDictionary<double, double> input, double start, double step, double end)
        {
            /*
                To get dot's range, used Lagrange polinom:
                y = sum(y[i]*l[i](x)), where i = 1..input.Count and
                l[i](x) = mult((x - x[j])/(x[i] - x[j])), where j = 1..input.Count, j!=i
            */
            
            //Result range
            SortedDictionary<double, double> result = new SortedDictionary<double, double>();
            //For each dot in result range
            for (double i = start; i <= end; i += step)
            {
                //Result
                double res = 0;
                //y[i]*(x - x[0])*...*(x - x[n])/(x[i] - x[0])*...*(x[i] - x[n])
                foreach (KeyValuePair<double, double> p in input)
                {
                    //(x[i] - x[0])*...*(x[i] - x[n])
                    double denom = 1;
                    ...
                    foreach (KeyValuePair<double, double> k in input)
                        if (k.Key != p.Key)
                        {
                            denom *= p.Key - k.Key;
                            numer *= i - k.Key;
                        }
                    res += p.Value * numer / denom;
                }
                result.Add(i, res);
            }
            return new KeyValuePair<Lines, SortedDictionary<double, double>> (Lines.Lagrang, result);
        }
    }
}